// test_mqtt_web.mjs —— 网页端端到端模拟：加载 web/index.html 的真实 JS，
// 配一个行为与 main.py 一致的假 BLE 设备，验证 WiFi 面板
// （扫描列表渲染 / 点击填入 / 连接成功与失败 / 状态刷新 / 断开 / 忘记）与 MQTT 面板
// （保存配置 / 状态刷新 / 忘记配置），AP 面板已移除。
// 运行：node test/test_mqtt_web.mjs （需 Node 18+，无需浏览器）
import fs from 'fs';
import vm from 'vm';
...
/* ---------- 假设备（行为对齐 main.py 的 WiFi/MQTT/认证/文件命令） ---------- */
function makeDevice() {
  const dev = {
    name: 'ESP32S3-MQTT',
    gatt: { connected: true },
  ...
  dev.handleCmd = line => {
    ...
    if (line === 'PING') return 'PONG ESP32S3-MQTT 1.0\nDONE\n';
    ...
    if (line.startsWith('MQTT_SET ')) {
      const parts = line.slice(9).split(' ');
      if (parts.length < 4) return 'ERR bad args\n';
      const port = +parts[1];
      if (!(port >= 1 && port <= 65535)) return 'ERR bad port\n';
      return 'OK mqtt configured\nDONE\n';
    }
    if (line === 'MQTT_FORGET') return 'OK mqtt forgotten\nDONE\n';
    if (line === 'MQTT_STATUS') {
      return 'MQTT CONNECTED\n' +
             'MQTT_CFG ' + b64('mqtt.example.com') + ' 8883 ' + b64('testuser') + '\n' +
             'MQTT_LED ' + b64('esp32s3/led') + '\nDONE\n';
    }
    if (line === 'WIFI_STATUS') {
...
expect('MQTT 状态已刷新（连接后自动）', logs.some(l => l.includes('MQTT 状态已刷新')));
expect('WiFi 状态面板显示已连接 MyWiFi', (els.wifiStatus.textContent || '').includes('已连接 MyWiFi'));
expect('MQTT 状态面板显示 CONNECTED 与 broker', (els.mqttStatus.textContent || '').includes('CONNECTED') && (els.mqttStatus.textContent || '').includes('mqtt.example.com:8883'));
expect('MQTT 状态面板显示灯珠主题', (els.mqttStatus.textContent || '').includes('esp32s3/led'));
expect('WiFi 状态面板不再显示热点', !(els.wifiStatus.textContent || '').includes('热点'));
...
await vm.runInContext('refreshMqttStatus()', sandbox);
await flush();
expect('MQTT 状态刷新正常', logs.some(l => l.includes('MQTT 状态已刷新')));
...
/* ===== 场景 9：MQTT 保存配置 ===== */
console.log('\n[场景 9] MQTT 保存配置');
vm.runInContext('document.getElementById("mqttHost").value = "mqtt.example.com"', sandbox);
vm.runInContext('document.getElementById("mqttPort").value = "8883"', sandbox);
vm.runInContext('document.getElementById("mqttUser").value = "testuser"', sandbox);
vm.runInContext('document.getElementById("mqttPwd").value = "secret"', sandbox);
await vm.runInContext('saveMqtt()', sandbox);
await flush();
expect('MQTT 配置保存日志（含 host:port）', logs.some(l => l.includes('MQTT 配置已保存: mqtt.example.com:8883')));
expect('保存后自动刷新状态', logs.some(l => l.includes('MQTT 状态已刷新')));

/* ===== 场景 10：MQTT 保存校验（空 host / 坏端口） ===== */
console.log('\n[场景 10] MQTT 输入校验');
vm.runInContext('document.getElementById("mqttHost").value = ""', sandbox);
await vm.runInContext('saveMqtt()', sandbox);
await flush();
expect('空 host 被拒', logs.some(l => l.includes('请填写 MQTT broker 地址')));
vm.runInContext('document.getElementById("mqttHost").value = "h"', sandbox);
vm.runInContext('document.getElementById("mqttPort").value = "70000"', sandbox);
await vm.runInContext('saveMqtt()', sandbox);
await flush();
expect('坏端口被拒', logs.some(l => l.includes('端口无效')));

/* ===== 场景 11：MQTT 忘记配置 ===== */
console.log('\n[场景 11] MQTT 忘记配置');
await vm.runInContext('forgetMqtt()', sandbox);
await flush();
expect('忘记 MQTT 日志', logs.some(l => l.includes('已删除 MQTT 配置')));
